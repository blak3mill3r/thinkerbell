...
{
  // param 1: struct dNeurons A
  kernel_weight_sample.setParameter(0, static_cast<int>(m_B->m_neurons.size) );
  kernel_weight_sample.setParameter(4, m_B->m_device_memory.ptr() );
  // param 2: struct dNeurons B
  kernel_weight_sample.setParameter(8,  static_cast<int>(m_A->m_neurons.size) );
  kernel_weight_sample.setParameter(12, m_A->m_device_memory.ptr() );
  // param 3: weight_type * W
  ...
  kernel_weight_sample.launch(1, 1, stream);
}

void Rbm::weight_update( const cuda::Stream &stream )
{
  // param 1: struct dNeurons A
  kernel_weight_update.setParameter(0, static_cast<int>(m_B->m_neurons.size) );
  kernel_weight_update.setParameter(4, m_B->m_device_memory.ptr() );
  // param 2: struct dNeurons B
  kernel_weight_update.setParameter(8,  static_cast<int>(m_A->m_neurons.size) );
  kernel_weight_update.setParameter(12, m_A->m_device_memory.ptr() );
  // param 3: weight_type * W
  ...
  kernel_weight_update.launch(1, 1, stream);
}
